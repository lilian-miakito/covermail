...
def test_v2_prompt_declares_exact_primer_without_a_sentence_limit() -> None:
    primer = "Je voulais te raconter calmement ce qui s'est passé."
    messages = logical_messages_v2(_cover(), "Le jardin", primer)
    ...
    tokenizer = RecordingTokenizer()
    assert render_chat_prompt_v2(tokenizer, _cover(), "Le jardin", primer) == "rendered"
    assert tokenizer.call["date_string"] == LLAMA_3_PINNED_DATE
